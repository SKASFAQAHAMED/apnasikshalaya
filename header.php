...
if ($Role == "teacher") {
  ...
  $sql = "SELECT id, nameIs, thumbnailIs FROM apnaStudents WHERE emailIs = ? && passIs = ?;";
  $stmt = $con->stmt_init();
  $stmt->prepare($sql);
  $stmt->bind_param("ss", $user, $pass);
  $stmt->execute();
  ...
  if ($stmt->num_rows() == 1) {
    $loggedin = true;
    $stmt->store_result();
    $stmt->bind_result($studentId, $studentName, $dp);
    $stmt->fetch();
  } else {
    $loggedin = false;
  }
  ...
  <div class="row">
    ...
    <ul class="navbar-nav mx-auto mb-2 mb-lg-0">
            <li class="nav-item dropdown">
              <button id="navbarDropdownMenuLink" role="button" data-bs-toggle="dropdown" aria-expanded="false" class="wow animate__animated nav-arrow animate__fadeInUp smoothScroll arrow-btn" data-toggle="modal" data-target="#cataModel"><i class="fas fa-book"></i></button>
              <ul class="dropdown-menu dropdown-menu_mid" aria-labelledby="navbarDropdownMenuLink">
                <li><p>______________________________________________</p></li>
                <?php
                $show = "show";
                  $sql3 = "SELECT DISTINCT cataIs FROM apnaCourses WHERE extra = ?;";
                  $stmt3 = $con->stmt_init();
                  $stmt3->prepare($sql3);
                  $stmt3->bind_param("s", $show);
                  $stmt3->execute();
                  $stmt3->store_result();
                  $stmt3->bind_result($cata);
                  while($stmt3->fetch()) {
                    $sql4 = "SELECT DISTINCT subCataIs FROM apnaCourses WHERE extra = ? AND cataIs = ?;";
                    $stmt4 = $con->stmt_init();
                    $stmt4->prepare($sql4);
                    $stmt4->bind_param("ss", $show, $cata);
                    $stmt4->execute();
                    $stmt4->store_result();
                    if($stmt4->num_rows() != 0) {
                      echo '<li class="dropdown-submenu">
                        <a class="dropdown-item dropdown-toggle" href="/search.php?query='.$cata.'"><i class="fas fa-chevron-circle-left"></i> '.$cata.'</a>
                      ...
                      echo '<li><a class="dropdown-item" href="/search.php?query='.$cata.'">'.$cata.'</a></li>';
                    }
                  }
                ?>
                <li><p>______________________________________________</p></li>
    ...
    <!-- <div class="col-md-2 col-sm-2 col-xs-4 wow animate__animated animate__fadeInUp smoothScroll" style="text-align:center;">
      ...
      if ($loggedin && $Role == "student") {
        echo '<button class="arrow-btn nav-arrow"><a href="/student">';
        if (strlen($studentName) < 9) {
          $studentuserName  = explode(" ",$studentName);
          $usernameshow =  $studentuserName[0];
          echo $studentName;
        } else {
          $studentuserName  = explode(" ",$studentName);
          $usernameshow =  $studentuserName[0];
          echo substr($studentName, 0, 6) . '...';
        }
        echo '  <i class="fas fa-user-graduate"></i></a></button>';
      }
      if ($loggedin && $Role == "teacher") {
        echo '<button class="arrow-btn nav-arrow"><a href="/teacher/index.php">';
    ...
    <div class="col-md-0 col-sm-0 col-xs-2 wow animate__animated animate__fadeInUp smoothScroll mobile-show">
      <?php 
        if($loggedin) {
          echo '<button class="wow animate__animated nav-arrow animate__fadeInUp smoothScroll arrow-btn"><a href="/student"><i class="fas fa-user-circle"></i></a></button>';
        } else {
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="model-google">
          ...
          <a href="<?=$google_client->createAuthUrl()?>">google sign in testing</a>
        </div><br>
