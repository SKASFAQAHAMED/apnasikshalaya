...
<tr><th>#</th>
  ...
  <th>Delete</th>
    ...
    while ($stmt->fetch()) {
      echo '<tr>
      <td>' . $slno . '</td>
      <td class = "event_id" style="display:none;">' . $id . '</td>
      <td><a href="#" class="bg bg-primary sndmail_btn">' . $title . '</a></td>
      <td>' . $topic . '</td>
      <td>' . $enrolled . '</td>
      <td>' . $organize . '</td>
      <td>' . $keywords . '</td>
      <td>' . $eventdate . '</td>
      <td>
      <a href="#" class =" bg bg-primary view_btn"><i class="fas fa-eye"></i></a></td>
      <td><a href="#" class =" bg bg-info edit_btn"><i class="fas fa-edit"></i></a></td>
      <td><a href="#" class =" bg bg-danger delete_btn"><i class="fas fa-trash-alt"></i></a></td>
     </tr>';
      $slno++;
    }
    echo '</table>
  </div>';
  } elseif ($action == "viewsubs") {
    $eventid = mysqli_real_escape_string($con, htmlspecialchars($_GET['Eid'], ENT_QUOTES));
  ...
  <div class="panel-heading">List of Users of event-Id - ' . $eventid . '</div>
...
<div class="panel-heading">List of Students-Subscribed to </div>
<!--div class="panel-body">
...
<tr><th>#</th>
  ...
  <th>Phone-no.</th>
    ...
    $sql = "SELECT DISTINCT emailIs, nameIs, phoneIs FROM student_subs_event WHERE eventId = ?;";
    $stmt = $con->stmt_init();
    $stmt->prepare($sql);
