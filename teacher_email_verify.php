...
$sql = "SELECT verifyIs FROM apnaTeachers WHERE emailIs = ?;";
$stmt = $con->stmt_init();
$stmt->prepare($sql);
...
if($action == "mail_again") {
	...
	header("Location: /teacher_email_verify?email=$email&status=resend");
	exit();
} else {
	echo '<!DOCTYPE html>
	<head>
	<title>OTP verification for mentor | Apnasikshalaya</title>
	</head>';
	...
	echo '<div>
	<form action="teacher_profile" method="POST" class="form">
		...
		<br>
		<a class="center a" href="/teacher_email_verify?action=mail_again&email='.$email.'">Resend OTP</a>
